...
public class WrapMapFuncInHttpBytesFunc implements BytesFunc{
	
	public final MapFunc mapfunc;
	
	public WrapMapFuncInHttpBytesFunc(MapFunc mapfunc){
		this.mapfunc = mapfunc;
	}
	
	public byte[] call(byte[] in){
		return mapToBytes(mapfunc.call(bytesToMap(in)));
	}
	
	/** Must be http request or response bytes */
	public Map bytesToMap(byte b[]){
		Map m = new HashMap();
	...
	public byte[] mapToBytes(Map<Object,Object> m){
		try{
	...
	public static byte[] asBytes(Object ob){
		...
		throw new RuntimeException("Not byte[] or String: "+ob);
	}
